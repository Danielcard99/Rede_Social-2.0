...
class HomeModel
{
    public static function postFeed($post)
    {
        ...
        if (preg_match('/\[imagem/', $post)) {
            $post = preg_replace("/(.*?)\[imagem=(.*?)\]/", "<p>$1<img></img src=$2></p>", $post);
        } else {
            $post = '<p>' . $post . '</p>';
        }
    ...
    public static function retrieveFriendsPosts()
    {
        ...
        foreach ($amigosConfirmados as $key => $value) {
            $listaAmigos[$key]['id'] = \Classes\Models\UsuariosModel::getUsuarioById($value)['id'];
            $listaAmigos[$key]['nome'] = \Classes\Models\UsuariosModel::getUsuarioById($value)['nome'];
            $listaAmigos[$key]['email'] = \Classes\Models\UsuariosModel::getUsuarioById($value)['email'];
            $listaAmigos[$key]['img'] = \Classes\Models\UsuariosModel::getUsuarioById($value)['img'];
            $listaAmigos[$key]['ultimo_post'] = \Classes\Models\UsuariosModel::getUsuarioById($value)['ultimo_post'];
        }
